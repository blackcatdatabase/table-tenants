...
namespace BlackCat\Database\Packages\Tenants\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\Tenants\Dto\TenantDto;
use BlackCat\Database\Packages\Tenants\Mapper\TenantDtoMapper;
 ...
 */
final class TenantsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private TenantRepository $tenantRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'tenants' => $this->tenantRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\Tenants\ModuleException("Record {$idStr} not found for locking.");
            }
